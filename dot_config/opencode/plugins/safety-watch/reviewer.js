...
export function createReviewer({ client, directory, state, model, contextTokens, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  ...
  function scheduleCompaction(sessionID, threshold) {
    ...
    if (!current || !Number.isFinite(limit) || current.inputTokens < limit * threshold || compactions.has(sessionID)) return;
    clearIdleCompaction(sessionID);
    const compacting = client.session.summarize({
      path: { id: current.reviewerID }, query: { directory }, body: { ...current.model, auto: true },
    }).catch(() => {}).finally(() => {
  ...
  function scheduleIdleCompaction(sessionID) {
    ...
    timers.set(sessionID, {
      short: setTimeout(() => scheduleCompaction(sessionID, SHORT_IDLE_COMPACTION_RATIO), SHORT_IDLE_MS),
      long: setTimeout(() => scheduleCompaction(sessionID, LONG_IDLE_COMPACTION_RATIO), LONG_IDLE_MS),
    });
  }
  ...
  async function reviewerSession(parentID) {
    ...
    if (typeof persisted === "string") {
      const response = await client.session.get({ path: { id: persisted }, query: { directory } });
      if (response?.data) {
    ...
    const created = unwrap(await client.session.create({
      body: { parentID, title: "[internal] Safety Watch reviewer" }, query: { directory },
  ...
  async function queueReview(sessionID, task) {
    ...
    if ((generations.get(sessionID) ?? 0) !== generation) throw new Error("Safety Watch review was canceled");
  ...
  async function review(sessionID, args) {
    ...
    const toolIDs = unwrap(await client.tool.ids({ query: { directory } }), "listing reviewer tools");
    const tools = Object.fromEntries(toolIDs.map((id) => [id, false]));
    const selectedModel = model();
    const reviewerModel = selectedModel ? { providerID: selectedModel.split("/")[0], modelID: selectedModel.split("/").slice(1).join("/") } : undefined;
    ...
    async function prompt(text) {
      ...
      let timeout;
      return Promise.race([
        client.session.prompt({ path: { id: reviewerID }, query: { directory }, body: {
          agent: REVIEWER_AGENT, model: reviewerModel, tools, system: REVIEWER_PROMPT, parts: [{ type: "text", text }],
        } }),
        new Promise((resolve) => { timeout = setTimeout(() => resolve(), remaining); }),
    ...
    async function decide(text, schedule = true) {
      ...
      if (schedule) {
        usage.set(sessionID, { reviewerID, model: reviewerModel, inputTokens: message.info?.tokens?.input ?? 0 });
    ...
    try {
      const decision = await decide(`# Determine if this is safe to run:\n${args}`);
      if (decision) return decision;
    } catch {
      const decision = await decide(`Answer with this shape only and no other text:\n${RESPONSE_SHAPE}`, false);
      if (decision) return decision;
    }
  ...
  async function cancelReview(sessionID) {
    ...
    if (reviewerID) await client.session.abort({ path: { id: reviewerID }, query: { directory } }).catch(() => {});
  }
  ...
  return { isReviewer: (sessionID) => owners.has(sessionID), clearIdleCompaction, scheduleIdleCompaction, queueReview, review, cancelReview, handleDeleted };
}
