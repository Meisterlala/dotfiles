...
1. Search before creating with `agentgateway_plane_search_work_items`. Do not create duplicates.
...
4. Before starting or changing an existing item, read it with `agentgateway_plane_retrieve_work_item` (or `agentgateway_plane_retrieve_work_item_by_identifier` when you have the human identifier) and inspect `agentgateway_plane_list_work_item_comments`. Use `agentgateway_plane_list_work_item_activities` only when change history is relevant.
...
1. Search the project with `agentgateway_plane_search_work_items` using component names, important nouns, error text, and the intended outcome.
...
4. If a duplicate was already created, cancel it with `agentgateway_plane_update_work_item` and add a short comment naming the canonical item.
5. If work is related but independently actionable, create a separate item and name the related item in its description or comment.

Different wording does not imply different work.

## Search requests

`agentgateway_plane_search_work_items` searches work-item names, sequence IDs,
and project identifiers. It does not filter by state.

Treat minor spelling errors such as "Sreach for open issues" as a request to
search for open work items. Do not search for the literal word `open`: it is a
state filter, not a reliable title keyword.

For a request such as "search for open issues" with a component, error text,
project identifier, or other concrete terms:

1. Search using the supplied component, error text, project identifier, or other
   concrete terms.
2. Retrieve each plausible result and resolve its `state` with
   `agentgateway_plane_list_states` for that result's project.
3. Report only items whose state group is `backlog`, `unstarted`, or `started`.

For every open-item request without concrete search terms, list work items for
the relevant project without PQL. Use the default project when none is named.
Request a sparse fieldset including `state` and `project`, use up to 100 results
per page, and continue through `next_cursor` until pagination is complete.
Resolve the project's states with `agentgateway_plane_list_states`, then filter
the returned items client-side to state groups `backlog`, `unstarted`, and
`started`. Report the scope and pagination status.
...
Name the canonical or superseding item in the cancellation comment.
...
For a meaningfully separate regression, prefer a new bug over rewriting the history of the completed item.
...
Record a dependency, duplicate, or related item in the description or a durable
comment. Use a parent item where nesting represents the work accurately.
...
3. Resolve only the IDs you actually need:
...
5. When this policy requires durable discussion, use `agentgateway_plane_create_work_item_comment`.
6. Retrieve the item again with `agentgateway_plane_retrieve_work_item` to confirm the resulting state.

For a new item:

1. Search for duplicates with `agentgateway_plane_search_work_items`.
...
- Are follow-ups truly separate and recorded?
